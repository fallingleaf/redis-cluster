...
class StrictRedis(object):
    ...
    def __init__(self, hosts, socket_timeout=None,
                 ...
                 max_connections=None, cluster=True):
        ...
        if cluster:
            self.db = Cluster(hosts, **kwargs)
        else:
            self.db = pool_class(hosts, **kwargs)
    ...
    def __repr__(self):
        return "%s<%s>" % (type(self).__name__, repr(self.db))
    ...
    def pubsub(self, **kwargs):
        ...
        return PubSub(pool, **kwargs)

    # COMMAND EXECUTION AND PROTOCOL PARSING
    def execute_command(self, *args, **options):
        "Execute a command and return a parsed response"
        return self.db.execute_command(*args, **options)
